...
describe("processNumber function", () => {
  it("Adds number character to the last number in formula.", () => {
    ...
    const expected = "22";
    const actual = processNumber(char, formula, locationTrail).getLastElement()
      .value;
    assert.equal(actual, expected);
  });
  it("Adds number character to the last number in formula through locationTrail.", () => {
    ...
    const expected = "45";
    const actual = processNumber(char, formula, locationTrail)
  ...
  it("Throws exception when trying to add a point to a number that already has one .", () => {
    ...
    const fn = () => processNumber(char, formula, locationTrail);
